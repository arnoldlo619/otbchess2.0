...
The Club brand trigger was then reviewed in both states. It preserves a fixed **64px** height while the original compact OTB!! mark crossfades and lightly scales down as the supplied green checkered OTB!! wordmark fades and translates into the expanded **152px** visual field. The reveal starts after a short 75ms delay, so the rail begins opening before the wordmark appears; collapse reverses immediately. Both image layers are decorative within the already-labelled **Back to all clubs** button, and `prefers-reduced-motion` continues to remove the transition.
