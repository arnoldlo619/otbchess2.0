...
This QA record covers the Club Feed post composer refinement: its compact-to-expanded sharing hierarchy, light and dark token model, responsive action layout, real attachment workflow, discard flow, and post submission contract. The latest refinement places the member identity in a dedicated header row and gives the expanded writing field the full composer width. It also removes the decorative border-trace animation in favor of a static, theme-aware focus treatment.
