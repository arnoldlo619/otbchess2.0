...
- [x] Club Profile: Add focused visitor/member navigation and parent-tab routing regression coverage
...
- [x] Club Profile: Add rendered checks that members retain direct Album and League rail access and visitors do not see standalone entries
...
- [x] Tournament payments: Remove Optional entry payment links and Player payment order/instructions sections from the Tournament Wizard
...
- [x] Director presentation: Remove redundant selected-result checks and compact board-result control height
...
- [x] Matchup Prep: Obtain authoritative CI production-build evidence after local Vite build exhausts the sandbox 1 GB Node heap
...
- [x] CI unblock: Run the full suite and obtain a green GitHub production build for the Matchup Prep launch assessment
...
- [x] Migrate all raw fetch() calls in Director.tsx, Tournament.tsx, and Report.tsx to apiFetch (Bearer token auth for production) — Director.tsx: 8 calls migrated (push notifications x4, players snapshot, /end x2, DELETE tournament); Tournament.tsx + Report.tsx: 0 raw fetch calls found
- [x] Comprehensive audit: migrate all remaining raw fetch() calls across all client/src pages and components to authFetch — 137 calls migrated across 47 files (clubsApi.ts 13, clubBattleApi.ts 1, AdminStaff.tsx 1, OpeningsAdmin.tsx 1, MatchupPrep.tsx 1, + 119 from script + 1 Director.tsx public); 19 remaining are external APIs (chess.com, lichess), test files, or CORS image fetches
- [x] Swiss+Elim eliminated banner: show player's final Swiss score (e.g., "3.5 / 5 pts") alongside placement
...
- [x] P2: Player card labels — replace "Champion/1st of 16" with "Quad 1 Champion/2nd in Quad 2" etc.
- [x] P2: Mobile responsive — QuadsDirectorPanel touch targets 44px+, horizontal Quad tabs, sticky header
- [x] P2: Accessibility — semantic buttons, keyboard nav, focus indicators, ARIA labels, non-color states
...
- [x] Add "Between Rounds", "Ready to Start", "Awaiting Finalization", "Draft", "Cancelled" display states
- [x] Display status through text, icon, and color on all surfaces
- [x] Show save/sync state separately from tournament state
- [x] Finalization states: pending, success, error, idempotent retry
...
- [x] Status band: text + icon + color, never contradictory
...
- [x] Do not expose future-round controls as actionable
...
- [x] Mathematically accurate scores, placements, draw rate in reports
- [x] Tiebreak legend is format-aware: Buchholz for Swiss, H2H + Sonneborn-Berger for Quads

### K. Responsive Director Operation
- [x] Verify director workflow at 360px, 390px, 768px, 1024px, 1440px
- [x] No critical action hidden behind hover-only interaction
...
- [ ] Safe confirmation for deletion
- [ ] No rough transition between banner/sidebar/feed
- [ ] No oversized blank header consuming first viewport

### E. Club Creation
- [ ] Progressive flow: name → location → type → description → image → public/private → review
- [ ] Slug preview
- [ ] Duplicate-name feedback
- [ ] Image crop guidance
- [ ] Clear public visibility explanation
...
- [x] Make tournament player-report cards and prize-template edit controls native keyboard-operable buttons with regression coverage
...
- [x] No unnecessary full-screen route loader
...
### L. CI Quality Gates
- [x] TypeScript check in active GitHub CI (0 errors)
- [x] ESLint in active GitHub CI (0 errors)
- [x] Unit tests in active GitHub CI (all pass)
...
- [x] Internal link validation in active GitHub CI
- [x] Add deterministic repository-wide literal internal-link validation to the active CI unit-test gate
- [x] Performance budget check documented
- [x] Enforce documented total-JS, largest-chunk, and CSS artifact budgets after the active CI production build
- [x] Sync the verified source to GitHub, preserve the CI template, and document owner-authorized workflow activation
- [x] Activate `docs/CI_WORKFLOW_TEMPLATE.yml` as `.github/workflows/ci.yml` with owner-authorized workflow permission and obtain a green run
...
- [x] Quads lifecycle regression suite: remove stale unused fixtures while retaining section, tiebreak, and roster-mutation coverage
- [x] Chess provider proxy: type CORS and rate-limit request boundaries while preserving origin and IP-key behavior
- [x] Broadcast service: remove stale payload fields and type optional creator attribution without changing live event delivery
...
- [x] League authorization: type the shared middleware userId lookup in the request helper without changing unauthorized outcomes
...
- [x] Server entrypoint: remove confirmed unused legacy proxy, limiter declarations, and schema import without altering mounted route behavior
- [x] Tournament analytics: type the shared authenticated user boundary for owner-only analytics reads
...
- [x] Registration modal: remove unused player aliases, time-control state, and player-card side binding without changing session flows
- [x] Registration modal: guard QR and direct-session creation errors as unknown while preserving clear fallback messaging
- [x] Registration modal: replace direct avatar fallback DOM mutation with React-managed initials fallback
...
- [x] Move Tree Panel: remove unused callback hook without changing variation navigation
- [x] Pairing Swap modal: replace non-semantic backdrop handlers with a semantic close control
...
- [x] Swiss Phase Summary: remove unused chevron icon without changing bracket summary behavior
- [x] Deliver an evidence-based reliability status assessment covering verified work, remaining backlog, deferred payment scope, and release risks
- [x] Retire the Chessnut beta feature: remove device UI, routes, integrations, and stale references while preserving manual tournament and game flows
- [x] Audit and complete the Repertoire Builder end to end with premium user workflows and extensive curated opening-line coverage
...
- [x] Expand the Black library with source-backed practical Sicilian systems and King's Indian Defense canonical branches
...
- [x] P0-1e: Audit and fix all broken CTAs (Host Tournament, View Live Demo, Tools, footer links)
...
- [x] P0-5: Fix draw rate formula in Report.tsx to use completedGames as denominator; fix generateResultsPdf to skip Buchholz page for quads
- [x] P0-6: Fix nested interactive elements in Pricing.tsx and other pages (Link wrapping button)
- [x] Testing: Add Playwright coverage for six core public routes, mobile/desktop overflow, create redirect, landing host CTA, auth tabs, and join entry
- [x] P0-7: Fix duplicate H1 in tournament report export cards (aria-hidden on export-only clones)
- [x] P0-8: Verify homepage zero-stat flash is fixed (already has skeleton + FLOORS)
- [x] P0-9: Add unit tests for format label, status selector, draw rate, quads tiebreaks
...
## Matchup Prep Analysis Workspace — Release Remediation
- [x] Audit each completed analysis-workspace requirement against real report persistence, routing, access, cache, and active-play flows
- [x] Persist or resolve authoritative completed source-game PGN so source-game analysis can verify report membership and replay without trusting browser data
...
- [x] Audit prior Lichess report, analysis, scheduler, cache, database, and operations architecture against the three-layer contract
- [x] Add immutable population-reference contracts, local aggregate manifests, tracked-position registry, and safe schema migrations
- [x] Implement strict official Lichess population Explorer fallback with trusted legality, caching, coalescing, and cooldown controls
- [x] Implement safe archive catalog/checksum discovery, batch-only staging/publishing/rollback primitives, and an explicit full-ingestion runtime boundary
- [x] Attach turn-aware eligible population context to immutable Matchup Prep reports with methodology disclosure
- [x] Add protected population dataset health and diagnostics visibility for operators
- [x] Add deterministic archive, resolver, privacy, eligibility, and UI contracts
