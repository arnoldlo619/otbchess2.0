...
describe("White repertoire catalog expansion", () => {
  it("adds the four reviewed White systems from an authoritative named-opening source", () => {
    for (const slug of ["english-opening", "catalan-opening", "kings-indian-attack", "reti-opening", "ruy-lopez", "ponziani-opening", "trompowsky-attack", "scandinavian-defense", "sicilian-defense", "kings-indian-defense"]) {
      expect(seed).toContain(`slug: "${slug}"`);
    }
    expect(seed).toContain("limit: 20");
    expect(seed).toContain('color: "black"');
  ...
  it("reuses existing parent and line IDs rather than requiring missing slug indexes", () => {
    ...
    expect(reconcile).toContain('"scandinavian-defense"');
    expect(reconcile).toContain('"sicilian-defense"');
    expect(reconcile).toContain('"kings-indian-defense"');
    expect(reconcile).toContain("SET ol.color = o.color");
  });
