...
export default function LeagueHistory() {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-white dark:bg-[#0d1a0f]">
        <div className="flex flex-col items-center gap-3">
          ...
          <span className="text-sm text-[#6B6B50]">Loading season history…</span>
        </div>
      </div>
    );
  }
      ...
      <div className="min-h-screen flex items-center justify-center bg-white dark:bg-[#0d1a0f]">
        <div className="text-center">
          ...
          <Link href={`/leagues/${leagueId}`}>
            <button className="px-4 py-2 bg-[#4D6940] text-white rounded-lg">Back to League</button>
          </Link>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-[#FFF3D5] dark:bg-[#0d1a0f] text-[#1A1A1A] dark:text-white/90">
      ...
      <div className="relative h-44 sm:h-52 overflow-hidden bg-[#0d1a0f]">
        ...
        <div className="absolute top-4 left-4">
          <Link href={`/leagues/${leagueId}`}>
            <button className="flex items-center gap-1.5 text-sm text-[#6B6B50]/70 hover:text-white bg-black/30 backdrop-blur-sm px-3 py-1.5 rounded-lg transition-colors">
      ...
      <div className="max-w-6xl mx-auto px-4">
        <div className="relative -mt-10 mb-6 bg-white dark:bg-[#162118] border border-[#E8D9B0] dark:border-[#2a3d2e] rounded-2xl shadow-xl p-5">
          <div className="flex flex-col sm:flex-row sm:items-start gap-4">
            {/* Icon */}
            <div className="w-14 h-14 rounded-xl bg-gradient-to-br from-[#4D6940] to-[#2a4d33] flex items-center justify-center flex-shrink-0 shadow-lg">
              <Trophy className="w-7 h-7 text-white" />
            </div>
            {/* Name + meta */}
            <div className="flex-1 min-w-0">
              <h1 className="text-xl sm:text-2xl font-bold truncate">{league.name}</h1>
              <p className="text-sm text-[#6B6B50] dark:text-[#6B6B50] mt-0.5">
          ...
          {/* 4-stat row */}
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-5 pt-4 border-t border-[#E8D9B0]/70 dark:border-[#2a3d2e]">
            {[
              ...
              <div key={stat.label} className="flex flex-col gap-0.5">
                <span className="text-xs text-[#6B6B50] dark:text-[#6B6B50] flex items-center gap-1">
        ...
        <div className="flex gap-6 items-start pb-12">
          ...
          <div className="flex-1 min-w-0">
            {/* Tab bar */}
            <div className="flex gap-1 bg-[#E8D9B0]/40 dark:bg-[#162118] rounded-xl p-1 mb-6">
              {tabs.map((t) => (
                <button
                  key={t.id}
                  onClick={() => setTab(t.id)}
                  className={`flex-1 flex items-center justify-center gap-1.5 py-2.5 text-sm font-medium rounded-lg transition-all ${
                    tab === t.id
                      ? "bg-white dark:bg-[#1e3322] text-[#4D6940] shadow-sm"
                      : "text-[#6B6B50] hover:text-[#1A1A1A] dark:hover:text-[#6B6B50]/70"
                  }`}
                >
                  {t.icon}
                  ...
                  <div
                    ...
                    className={`flex items-center gap-3 p-3 rounded-xl transition-colors ${
                      i === 0 && isCompleted
                        ...
                        : "bg-[#FFF3D5]/70 dark:bg-[#162118] border border-transparent"
                    }`}
                  >
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold flex-shrink-0 ${
                      i === 0 ? "bg-amber-500 text-white" : i === 1 ? "bg-[#6B6B50] text-white" : i === 2 ? "bg-amber-700 text-white" : "bg-[#E8D9B0] dark:bg-white/10 text-[#6B6B50] dark:text-[#6B6B50]/70"
                    }`}>
                      {i === 0 && isCompleted ? <Crown className="w-4 h-4" /> : s.rank}
                    </div>
                    <div className="w-9 h-9 rounded-full bg-[#4D6940]/20 flex items-center justify-center text-sm font-bold flex-shrink-0">
                      {s.avatarUrl
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex gap-2 text-xs text-[#6B6B50] dark:text-[#6B6B50] mt-0.5">
                        <span>{s.wins}W-{s.draws}D-{s.losses}L</span>
                        {s.chesscomRating && <span className="text-green-500">{s.chesscomRating}</span>}
                      </div>
                    </div>
                    <div className="text-right flex-shrink-0">
                      <div className="text-lg font-bold text-[#4D6940]">{s.points}</div>
                      <div className="text-xs text-[#6B6B50]">pts</div>
                    </div>
                  </div>
                ))}
              </div>
            ...
            {tab === "weeks" && (
              <div className="space-y-3 mb-8">
                {weeks.map((w) => (
                  <div key={w.weekNumber} className="bg-[#FFF3D5]/70 dark:bg-[#162118] rounded-xl overflow-hidden border border-[#E8D9B0]/70 dark:border-[#2a3d2e]">
                    <button
                      className="w-full flex items-center justify-between p-4 hover:bg-[#E8D9B0]/50 dark:hover:bg-[#1e3322] transition-colors"
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-xs text-[#6B6B50] dark:text-[#6B6B50]">
                          {w.matches.filter((m) => m.result).length}/{w.matches.length} complete
                        </span>
                      </div>
                      {expandedWeek === w.weekNumber
                        ? <ChevronUp className="w-4 h-4 text-[#6B6B50]" />
                        : <ChevronDown className="w-4 h-4 text-[#6B6B50]" />
                      }
                    </button>
                    {expandedWeek === w.weekNumber && (
                      <div className="border-t border-[#E8D9B0]/70 dark:border-[#2a3d2e] divide-y divide-gray-100 dark:divide-[#2a3d2e]">
                        {w.matches.map((m) => (
                          <div key={m.id} className="flex items-center justify-between px-4 py-3 text-sm">
                            ...
                            <span className={`px-2.5 py-0.5 rounded-md font-mono text-xs font-bold flex-shrink-0 ${
                              m.result
                                ...
                                : "bg-[#E8D9B0]/40 dark:bg-white/10 text-[#6B6B50]"
                            }`}>
            ...
            {tab === "h2h" && (
              <div className="mb-8">
                <div className="mb-4">
                  <label className="block text-xs font-semibold text-[#6B6B50] dark:text-[#6B6B50] uppercase tracking-wider mb-1.5">
                    Select Player
                  </label>
                  <select
                    value={h2hPlayer ?? ""}
                    onChange={(e) => setH2hPlayer(e.target.value)}
                    className="w-full bg-[#FFF3D5]/70 dark:bg-[#162118] border border-[#E8D9B0] dark:border-[#2a3d2e] rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#4D6940]/50"
                  >
                    {standings.map((s) => (
                ...
                <div className="space-y-2">
                  {h2hRows.length === 0 ? (
                    <p className="text-center text-[#6B6B50] py-8">No head-to-head records yet</p>
                  ) : (
                    h2hRows.map((row) => (
                      <div key={row.opponentId} className="flex items-center gap-3 p-3 bg-[#FFF3D5]/70 dark:bg-[#162118] rounded-xl border border-[#E8D9B0]/70 dark:border-[#2a3d2e]">
                        <div className="w-9 h-9 rounded-full bg-[#4D6940]/20 flex items-center justify-center text-sm font-bold flex-shrink-0">
                          {row.opponentAvatar
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <span className="text-xs text-[#6B6B50] dark:text-[#6B6B50]">{row.total} games</span>
                        </div>
                        <div className="flex gap-1.5 text-xs font-semibold flex-shrink-0">
                          ...
                          <span className="px-2 py-0.5 bg-[#E8D9B0] dark:bg-white/10 text-[#6B6B50] rounded">{row.draws}D</span>
            ...
            {tab === "stats" && (
              <div className="space-y-4 mb-8">
                {/* Result distribution */}
                <div className="bg-[#FFF3D5]/70 dark:bg-[#162118] rounded-xl p-4 border border-[#E8D9B0]/70 dark:border-[#2a3d2e]">
                  <h3 className="font-semibold mb-3 flex items-center gap-2">
                    <BarChart3 className="w-4 h-4 text-[#4D6940]" /> Result Distribution
                  </h3>
                  <div className="space-y-2.5">
                    {[
                      { label: "White Wins", pct: seasonStats.whiteWinPct, count: seasonStats.whiteWins, color: "bg-white border border-[#E8D9B0]" },
                      { label: "Black Wins", pct: seasonStats.blackWinPct, count: seasonStats.blackWins, color: "bg-[#1A1A1A]" },
                      { label: "Draws", pct: seasonStats.drawPct, count: seasonStats.draws, color: "bg-[#4D6940]" },
                    ].map((row) => (
                      <div key={row.label}>
                        <div className="flex justify-between text-xs text-[#6B6B50] dark:text-[#6B6B50] mb-1">
                          <span>{row.label}</span>
                          <span>{row.count} ({row.pct.toFixed(1)}%)</span>
                        </div>
                        <div className="h-2 bg-[#E8D9B0] dark:bg-white/10 rounded-full overflow-hidden">
                          <div
                ...
                <div className="bg-[#FFF3D5]/70 dark:bg-[#162118] rounded-xl p-4 border border-[#E8D9B0]/70 dark:border-[#2a3d2e]">
                  <h3 className="font-semibold mb-3 flex items-center gap-2">
                    <TrendingUp className="w-4 h-4 text-[#4D6940]" /> Top Performers
                  </h3>
                  <div className="space-y-2">
                    {standings.slice(0, 3).map((s, i) => (
                      <div key={s.playerId} className="flex items-center gap-3">
                        <span className="text-xs font-bold text-[#6B6B50] w-4">#{i + 1}</span>
                        <div className="w-7 h-7 rounded-full bg-[#4D6940]/20 flex items-center justify-center text-xs font-bold">
                          {s.avatarUrl
                            ? <img src={s.avatarUrl} alt="" className="w-full h-full rounded-full object-cover" />
                            : s.displayName.charAt(0).toUpperCase()
                          }
                        ...
                        <span className="text-sm font-bold text-[#4D6940]">{s.points} pts</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="bg-[#FFF3D5]/70 dark:bg-[#162118] rounded-xl p-4 border border-[#E8D9B0]/70 dark:border-[#2a3d2e]">
                  <h3 className="font-semibold mb-3 flex items-center gap-2">
                    <Swords className="w-4 h-4 text-[#4D6940]" /> Season Summary
                  </h3>
                  <div className="grid grid-cols-2 gap-3 text-sm">
                    <div className="flex justify-between">
                      <span className="text-[#6B6B50]">Format</span>
                      <span className="font-medium">{league.formatType.replace("_", " ")}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-[#6B6B50]">Players</span>
                      <span className="font-medium">{league.maxPlayers}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-[#6B6B50]">Rounds</span>
                      <span className="font-medium">{league.totalWeeks}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-[#6B6B50]">Total Matches</span>
          ...
          <aside className="hidden lg:flex flex-col gap-4 w-72 flex-shrink-0">
            ...
            {champion && isCompleted && (
              <div className="bg-gradient-to-br from-amber-500/10 via-yellow-500/5 to-amber-500/10 border border-amber-500/30 rounded-2xl p-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-bold text-[#1A1A1A] dark:text-white">{champion.displayName}</p>
                    <p className="text-sm text-[#6B6B50] dark:text-[#6B6B50]">{champion.points} pts · {champion.wins}W-{champion.draws}D-{champion.losses}L</p>
                ...
                {champion.chesscomUsername && (
                  <a
                    ...
                    className="mt-3 flex items-center gap-1.5 text-xs text-[#4D6940] hover:text-[#4a8057] transition-colors"
                  >
            ...
            <div className="bg-white dark:bg-[#162118] border border-[#E8D9B0] dark:border-[#2a3d2e] rounded-2xl p-4">
              <div className="flex items-center gap-2 mb-3">
                ...
                {standings.slice(0, 8).map((s, i) => (
                  <div key={s.playerId} className="flex items-center gap-2 text-sm">
                    <span className={`w-5 h-5 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 ${
                      i === 0 ? "bg-amber-500 text-white" : i === 1 ? "bg-[#6B6B50] text-white" : i === 2 ? "bg-amber-700 text-white" : "bg-[#E8D9B0]/40 dark:bg-white/10 text-[#6B6B50]"
                    }`}>
                      {s.rank}
                    </span>
                    <span className="flex-1 truncate text-[#1A1A1A]/85 dark:text-[#6B6B50]/70">{s.displayName}</span>
                    <span className="font-bold text-[#4D6940] flex-shrink-0">{s.points}</span>
                  </div>
                ))}
                {standings.length > 8 && (
                  <button
                    onClick={() => setTab("standings")}
                    className="w-full text-xs text-[#4D6940] hover:text-[#4a8057] pt-1 transition-colors"
                  >
            ...
            <div className="bg-white dark:bg-[#162118] border border-[#E8D9B0] dark:border-[#2a3d2e] rounded-2xl p-4">
              <div className="flex items-center gap-2 mb-3">
                <BarChart3 className="w-4 h-4 text-[#4D6940]" />
            ...
            {/* Back to league */}
            <Link href={`/leagues/${leagueId}`}>
              <button className="w-full flex items-center justify-center gap-2 py-2.5 px-4 bg-[#4D6940] hover:bg-[#4a8057] text-white text-sm font-medium rounded-xl transition-colors">
