      ...
      <div
        className={`group block rounded-2xl overflow-hidden border transition-all duration-200 hover:scale-[1.02] hover:shadow-xl cursor-pointer ${
          isDark
            ...
            : "bg-white border-[#EEEED2] hover:border-[#3D6B47]/30 shadow-sm"
        }`}
      >
        {/* Board thumbnail */}
        <div className="relative">
          ...
          <div className="absolute top-2 left-2">
            ...
            {isEnded && (
              <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${isDark ? "bg-white/10 text-white/60" : "bg-gray-100 text-gray-500"}`}>
                ENDED
              </span>
            )}
          ...
          <div className="absolute top-2 right-2">
            <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${isDark ? "bg-black/40 text-white/70" : "bg-white/80 text-gray-600"}`}>
          ...
          {broadcast.lastMoveSan && isLive && (
            <div className="absolute bottom-2 right-2">
              <span className={`px-2 py-0.5 rounded-full text-[10px] font-mono font-bold ${isDark ? "bg-black/50 text-[oklch(0.75_0.14_145)]" : "bg-white/90 text-[#3D6B47]"}`}>
        ...
        <div className={`px-3 py-2.5 border-t ${isDark ? "border-white/06" : "border-[#EEEED2]"}`}>
          ...
          <div className="flex items-center justify-between mb-1">
            <div className="flex items-center gap-1.5 min-w-0">
              <span className={`w-2.5 h-2.5 rounded-sm flex-shrink-0 ${isDark ? "bg-white/80" : "bg-white border border-gray-300"}`} />
              <span className={`text-xs font-semibold truncate ${isDark ? "text-white/90" : "text-gray-800"}`}>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-1.5 min-w-0">
              <span className={`w-2.5 h-2.5 rounded-sm flex-shrink-0 ${isDark ? "bg-[oklch(0.25_0.07_145)] border border-white/20" : "bg-gray-800"}`} />
              <span className={`text-xs font-semibold truncate ${isDark ? "text-white/90" : "text-gray-800"}`}>
          ...
          <div className={`mt-2 pt-1.5 border-t flex items-center justify-between ${isDark ? "border-white/06" : "border-[#EEEED2]"}`}>
            {resultLabel ? (
              <span className={`text-[10px] font-semibold ${isDark ? "text-[oklch(0.65_0.14_145)]" : "text-[#3D6B47]"}`}>
                {resultLabel}
              </span>
            ) : (
            ...
            <ChevronRight className={`w-3 h-3 flex-shrink-0 transition-transform group-hover:translate-x-0.5 ${isDark ? "text-white/25" : "text-gray-300"}`} />
          </div>
        </div>
      </div>
    </Link>
    ...
    <div className={`rounded-2xl border overflow-hidden mb-6 ${isDark ? "bg-[oklch(0.20_0.06_145)] border-white/08" : "bg-[#F8FAF7] border-[#EEEED2]"}`}>
      ...
      <div className={`px-4 py-3 flex items-center justify-between border-b ${isDark ? "border-white/06" : "border-[#EEEED2]"}`}>
        <div className="flex items-center gap-2">
          <Radio className={`w-4 h-4 ${liveCount > 0 ? "text-[#4CAF50]" : isDark ? "text-white/40" : "text-gray-400"}`} />
        ...
        <Link href={`/tournament/${tournamentId}/broadcasts`}>
          <div className={`flex items-center gap-1 text-xs font-medium transition-colors ${isDark ? "text-white/40 hover:text-white/70" : "text-gray-400 hover:text-gray-600"}`}>
            View all
        ...
        {loading ? (
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
            {[1, 2, 3, 4].map((i) => (
              <div
                key={i}
                className={`rounded-2xl overflow-hidden animate-pulse ${isDark ? "bg-white/05" : "bg-gray-100"}`}
