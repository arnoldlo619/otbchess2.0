  ...
  return (
    <button
      ...
      className={`w-full flex items-center gap-2.5 px-3 py-2.5 rounded-xl border text-left transition-all ${
        selected
          ? isDark
            ? "border-[#436850]/70 bg-[#436850]/15 ring-1 ring-[#436850]/30"
          ...
          : "border-[#ADBC9F] bg-white hover:border-[#436850]/30 disabled:opacity-40"
      }`}
    >
      {/* Color dot */}
      ...
      {/* ELO */}
      {player.elo > 0 && (
        <span className={`text-xs font-mono flex-shrink-0 ${isDark ? "text-white/35" : "text-[#436850]"}`}>
          {player.elo}
        </span>
      )}
      {selected && (
    ...
    >
      ...
      >
        ...
        <div className={`flex items-center justify-between px-5 py-4 border-b flex-shrink-0 ${isDark ? "border-white/08" : "border-[#ADBC9F]/70"}`}>
          <div>
            <h2 className={`text-base font-bold flex items-center gap-2 ${isDark ? "text-white" : "text-[#12372A]"}`}>
        ...
        <div className={`flex items-center gap-2 px-5 py-3 border-b flex-shrink-0 ${isDark ? "border-white/06 bg-white/02" : "border-[#ADBC9F]/70 bg-[#FBFADA]/70/60"}`}>
          {/* Slot A */}
          <div className={`flex-1 px-3 py-2 rounded-xl border text-sm ${
            selectedA
              ? isDark ? "border-[#436850]/50 bg-[#436850]/10" : "border-[#436850]/30 bg-[#436850]/05"
              : isDark ? "border-white/08 bg-white/03" : "border-dashed border-[#ADBC9F] bg-white"
          }`}>
            {selectedA && playerA ? (
              <div>
                <p className={`font-semibold text-xs truncate ${isDark ? "text-white/90" : "text-[#12372A]"}`}>{playerA.name}</p>
                <p className={`text-[10px] ${isDark ? "text-white/35" : "text-[#436850]"}`}>Board {slotA?.game.board} · {slotA?.color}</p>
              </div>
            ) : (
              <p className={`text-xs ${isDark ? "text-white/25" : "text-[#436850]"}`}>Select player 1</p>
            )}
          </div>

          <ArrowLeftRight className={`w-4 h-4 flex-shrink-0 ${isDark ? "text-white/25" : "text-[#436850]/70"}`} />
          ...
          <div className={`flex-1 px-3 py-2 rounded-xl border text-sm ${
            ...
            {selectedB && playerB ? (
              <div>
                <p className={`font-semibold text-xs truncate ${isDark ? "text-white/90" : "text-[#12372A]"}`}>{playerB.name}</p>
                <p className={`text-[10px] ${isDark ? "text-white/35" : "text-[#436850]"}`}>Board {slotB?.game.board} · {slotB?.color}</p>
              </div>
            ) : (
              <p className={`text-xs ${isDark ? "text-white/25" : "text-[#436850]"}`}>Select player 2</p>
            )}
          </div>
        </div>

        {/* Search */}
        <div className={`px-5 py-3 border-b flex-shrink-0 ${isDark ? "border-white/06" : "border-[#ADBC9F]/70"}`}>
          <div className={`flex items-center gap-2 px-3 py-2 rounded-xl border ${
            isDark ? "bg-[oklch(0.25_0.07_145)] border-white/10" : "bg-white border-[#ADBC9F]"
          }`}>
            ...
            <input
              ...
              className={`flex-1 bg-transparent text-sm outline-none ${
                isDark ? "text-white placeholder:text-white/30" : "text-[#12372A] placeholder:text-[#436850]/60"
              }`}
            />
            {search && (
        ...
        <div className="flex-1 overflow-y-auto px-5 py-3 space-y-1.5">
          {filteredSlots.length === 0 ? (
            <div className="text-center py-8">
              <p className={`text-sm ${isDark ? "text-white/35" : "text-[#436850]"}`}>No players found</p>
            </div>
          ) : (
        ...
        {/* Preview */}
        {preview && (
          <div className={`px-5 py-3 border-t flex-shrink-0 ${isDark ? "border-white/08 bg-white/02" : "border-[#ADBC9F]/70 bg-[#FBFADA]/70/40"}`}>
            <p className={`text-[10px] font-bold uppercase tracking-wider mb-2 ${isDark ? "text-white/30" : "text-[#436850]"}`}>
              Preview after swap
            </p>
            {sameBoard && (
            ...
            <div className="grid grid-cols-2 gap-2 text-xs">
              {[preview.boardAAfter, preview.boardBAfter].filter((b, i, arr) => arr.findIndex(x => x.id === b.id) === i).map((board) => {
                ...
                return (
                  <div key={board.id} className={`px-3 py-2 rounded-xl border ${isDark ? "border-white/08 bg-white/03" : "border-[#ADBC9F] bg-white"}`}>
        ...
        {/* Footer */}
        <div className={`flex items-center justify-end gap-2.5 px-5 py-4 border-t flex-shrink-0 ${isDark ? "border-white/08" : "border-[#ADBC9F]/70"}`}>
          <button
            onClick={handleClose}
          ...
          <button
            ...
            className={`flex items-center gap-2 px-5 py-2 rounded-xl text-sm font-semibold transition-all disabled:opacity-40 ${
              isDark
                ? "bg-[#436850] text-white hover:bg-[#4CAF50] disabled:cursor-not-allowed"
                : "bg-[#436850] text-white hover:bg-[#2D5437] disabled:cursor-not-allowed"
            }`}
          >
