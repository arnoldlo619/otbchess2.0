...
function seedBadgeClass(seed: number | null, isDark: boolean): string {
  ...
  return isDark ? "bg-white/08 text-white/40 border-white/10" : "bg-[#FBFADA]/70 text-[#436850] border-[#ADBC9F]";
}
...
function MatchCard({ game, players, elimPlayers, isDark, isCurrentRound, myPlayerId, onEnterResult }: MatchCardProps) {
  ...
  const cardBorder = isMyGame
    ? isDark
      ...
      : "border-[#436850]/60 shadow-[0_0_0_2px_rgba(77,105,64,0.15)]"
    : isCurrentRound && isPending
      ? isDark ? "border-[#4CAF50]/35 shadow-[0_0_0_1px_rgba(76,175,80,0.15)]" : "border-[#436850]/35"
      : isDark ? "border-white/08" : "border-[#ADBC9F]/70";

  const winnerBg = isDark ? "bg-[#436850]/25" : "bg-[#436850]/07";
  const loserText = isDark ? "text-white/28" : "text-[#436850]/70";
  const divider = isDark ? "border-white/06" : "border-[#ADBC9F]/70";
    ...
    if (isByeRow) {
      return (
        <div className={`flex items-center gap-2 px-3 py-2.5 opacity-30`}>
          <span className={`text-[10px] font-bold w-6 text-center ${isDark ? "text-white/30" : "text-[#436850]/70"}`}>—</span>
          <span className={`text-xs italic ${isDark ? "text-white/30" : "text-[#436850]/70"}`}>BYE</span>
        </div>
      );
    }
      ...
      <div className={`relative flex items-center gap-2 px-3 py-2.5 transition-colors ${isWinner ? winnerBg : ""} ${shouldFlash ? "animate-bracket-winner-flash" : ""}`}>
        ...
        <div className="flex-1 min-w-0">
          <p className={`text-xs font-semibold truncate leading-tight ${
            isLoser ? loserText : isDark ? "text-white/88" : "text-[#12372A]"
          }`}>
            {player?.name ?? playerId}
          </p>
          {player?.elo && (
            <p className={`text-[9px] tabular-nums ${isDark ? "text-white/28" : "text-[#436850]"}`}>
              {player.elo}
            </p>
          )}
        </div>
        ...
        {score !== null && (
          <span className={`text-sm font-black tabular-nums flex-shrink-0 w-4 text-right ${
            isWinner ? (isDark ? "text-[#4CAF50]" : "text-[#436850]") : loserText
          }`}>
            {score}
          </span>
        )}
        ...
        {isMe && (
          <span className={`text-[8px] font-black px-1 py-0.5 rounded-full flex-shrink-0 ${
            isDark ? "bg-[#4CAF50]/20 text-[#4CAF50] border border-[#4CAF50]/30" : "bg-[#436850]/10 text-[#436850] border border-[#436850]/25"
          }`}>
            You
          </span>
        )}
          ...
          <button
            ...
            className={`flex-1 text-[10px] font-bold py-1.5 rounded-lg border transition-all active:scale-95 truncate ${
              isDark
                ...
                : "bg-[#FBFADA]/70 hover:bg-[#ADBC9F]/50 text-[#436850] border-[#ADBC9F]"
            }`}
...
function BracketConnector({
  ...
  const lineColor = isDark ? "rgba(76,175,80,0.18)" : "rgba(77,105,64,0.15)";
  const dotColor = isDark ? "rgba(76,175,80,0.35)" : "rgba(77,105,64,0.30)";
    ...
    <div
      className={`flex flex-col items-center justify-center rounded-xl border p-4 gap-3 flex-shrink-0 ${
        champ
          ...
          : "bg-[#FBFADA]/70 border-[#ADBC9F]/70"
      }`}
      style={{ width: 140 }}
    >
      <div className="relative">
        <Trophy className={`w-7 h-7 ${champ ? "text-amber-400" : isDark ? "text-white/15" : "text-[#436850]/50"}`} />
      ...
      {champ ? (
        <div className="text-center">
          ...
          <p className={`text-sm font-black leading-tight ${isDark ? "text-white" : "text-[#12372A]"}`}
            style={{ fontFamily: "'Clash Display', sans-serif" }}>
            {champ.name}
          </p>
          {seed && (
            <p className={`text-[9px] mt-0.5 ${isDark ? "text-white/35" : "text-[#436850]"}`}>
...
export function EliminationBracketView({
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      {thirdPlaceGame && (
        <div className={`rounded-2xl border p-4 ${isDark ? "bg-[oklch(0.18_0.04_145)] border-white/08" : "bg-[#FBFADA]/70 border-[#ADBC9F]/70"}`}>
          <div className="flex items-center gap-2 mb-3">
            <span className="text-lg leading-none">🥉</span>
            <span className={`text-[11px] font-black uppercase tracking-wider ${isDark ? "text-white/50" : "text-[#436850]"}`}>
              3rd Place Match
            </span>
          ...
          <p className={`text-[10px] mt-2 ${isDark ? "text-white/30" : "text-[#436850]"}`}>
      ...
      {allResultsIn && !isTournamentOver && !isLastRound && (
        <div className={`flex items-center gap-3 p-4 rounded-2xl border ${
          ...
          <div className="flex-1">
            <p className={`text-sm font-bold ${isDark ? "text-white" : "text-[#12372A]"}`}>All matches complete</p>
            <p className={`text-xs mt-0.5 ${isDark ? "text-white/45" : "text-[#436850]"}`}>
...
export function SwissElimCutoffScreen({
  ...
  const T = {
    card: isDark ? "bg-[oklch(0.22_0.06_145)] border-white/08" : "bg-white border-[#ADBC9F]/70",
    title: isDark ? "text-white" : "text-[#12372A]",
    sub: isDark ? "text-white/45" : "text-[#436850]",
    row: isDark ? "border-white/06 hover:bg-white/03" : "border-[#ADBC9F]/70 hover:bg-[#FBFADA]",
    rowIn: isDark ? "bg-[oklch(0.28_0.09_145)]/50" : "bg-[#436850]/05",
    rankBadge: isDark ? "bg-white/08 text-white/50" : "bg-[#ADBC9F]/40 text-[#436850]",
  ...
  return (
    <div className={`rounded-2xl border overflow-hidden ${T.card}`}>
      <div className={`px-5 py-4 border-b ${isDark ? "border-white/06" : "border-[#ADBC9F]/70"}`}>
          ...
          return (
            <React.Fragment key={row.player.id}>
              <div className={`flex items-center gap-3 px-5 py-2.5 transition-colors ${advances ? T.rowIn : T.row}`}>
                ...
                <span className={`text-xs tabular-nums font-bold ${isDark ? "text-white/60" : "text-[#436850]"}`}>
                  {row.points}
                </span>
                <span className={`text-[10px] tabular-nums ${isDark ? "text-white/30" : "text-[#436850]"}`}>
