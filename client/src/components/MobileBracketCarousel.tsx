...
function MobileMatchCard({
  ...
  const cardBorder = isMyGame
    ? isDark
      ...
      : "border-[#436850]/60 shadow-[0_0_0_2px_rgba(61,107,71,0.15)]"
    : isCurrentRound && isPending
      ? isDark ? "border-[#4CAF50]/40 shadow-[0_0_0_1px_rgba(76,175,80,0.12)]" : "border-[#436850]/35"
    ...
    if (isByeRow) {
      return (
        <div className="flex items-center gap-3 px-4 py-3 opacity-30">
          ...
          <span className={`text-sm italic ${isDark ? "text-white/30" : "text-[#436850]/70"}`}>BYE</span>
        </div>
      );
    }
      ...
      <div className={`relative flex items-center gap-3 px-4 py-3 transition-colors ${isWinner ? winnerBg : ""} ${shouldFlash ? "animate-bracket-winner-flash" : ""}`}>
        ...
        <div className={`w-8 h-8 rounded-full flex-shrink-0 flex items-center justify-center text-xs font-black ${
          isLoser
            ...
            : isDark ? "bg-[#436850]/40 text-[#4CAF50]" : "bg-[#436850]/10 text-[#436850]"
        }`}>
        ...
        <div className="flex-1 min-w-0">
          <p className={`text-sm font-semibold truncate leading-tight ${
            isLoser ? loserText : isDark ? "text-white/90" : "text-[#12372A]"
          }`}>
            {player?.name ?? playerId}
          </p>
          {player?.elo && (
            <p className={`text-[11px] tabular-nums ${isDark ? "text-white/30" : "text-[#436850]"}`}>
              {player.elo}
            </p>
          )}
        </div>
    ...
    <div className={`rounded-2xl border overflow-hidden ${cardBg} ${cardBorder} ${isBye ? "opacity-50" : ""}`}>
      ...
      <div className={`flex items-center justify-between px-4 py-2 border-b ${divider}`}>
        <div className="flex items-center gap-2">
          <span className={`text-[10px] font-bold uppercase tracking-widest ${isDark ? "text-white/30" : "text-[#436850]"}`}>
            Match {matchNumber}
          </span>
          {isMyGame && (
  ...
  return (
    <div className={`flex flex-col items-center justify-center gap-5 py-10 px-6 rounded-2xl border min-h-[260px] ${
      champ
        ...
        : isDark ? "bg-[oklch(0.20_0.05_145)] border-white/06" : "bg-[#FBFADA]/70 border-[#ADBC9F]"
    }`}>
      <div className="relative">
        <div className={`w-20 h-20 rounded-full flex items-center justify-center ${
          champ ? "bg-amber-400/20" : isDark ? "bg-white/06" : "bg-[#ADBC9F]/40"
        }`}>
          <Trophy className={`w-10 h-10 ${champ ? "text-amber-400" : isDark ? "text-white/20" : "text-[#436850]/70"}`} />
        </div>
        {champ && (
      ...
      {champ ? (
        <div className="text-center">
          ...
          <p className={`text-2xl font-black leading-tight mb-1 ${isDark ? "text-white" : "text-[#12372A]"}`}
...
export function MobileBracketCarousel({
  ...
  const T = {
    ...
    tabBg: isDark ? "bg-[oklch(0.18_0.05_145)]" : "bg-[#ADBC9F]/40",
    tabActive: isDark ? "bg-[oklch(0.32_0.10_145)] text-white shadow-sm" : "bg-white text-[#12372A] shadow-sm",
    tabInactive: isDark ? "text-white/40" : "text-[#436850]",
    navBtn: isDark ? "bg-white/08 hover:bg-white/14 text-white/60 border-white/08" : "bg-white hover:bg-[#FBFADA] text-[#436850] border-[#ADBC9F]",
  };
  ...
  if (rounds.length === 0) {
    return (
      <div className={`flex flex-col items-center justify-center py-16 gap-3 rounded-2xl ${
        ...
        <Trophy className={`w-10 h-10 ${isDark ? "text-white/20" : "text-[#436850]/70"}`} />
        <p className={`text-sm font-semibold ${isDark ? "text-white/30" : "text-[#436850]"}`}>No bracket yet</p>
      </div>
    );
  }
            ...
            return (
              <div key={round.number} className="w-full flex-shrink-0 flex flex-col gap-3 px-0.5">
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <h3 className={`text-base font-black ${isDark ? "text-white" : "text-[#12372A]"}`}
                      style={{ fontFamily: "'Clash Display', sans-serif" }}>
                      {roundLabels[idx]}
                    </h3>
          ...
          {Array.from({ length: totalSlides }).map((_, i) => (
            <button
              ...
              className={`rounded-full transition-all ${
                i === slideIdx
                  ? isDark ? "w-4 h-2 bg-[#4CAF50]" : "w-4 h-2 bg-[#436850]"
                  : isDark ? "w-2 h-2 bg-white/20" : "w-2 h-2 bg-[#ADBC9F]"
              }`}
            />
          ))}
        </div>
