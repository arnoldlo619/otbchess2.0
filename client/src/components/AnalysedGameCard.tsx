...
function getResultConfig(result: string | null, isDark: boolean) {
  switch (result) {
    case "1-0":
      return { label: "1 – 0", bg: isDark ? "bg-white/10" : "bg-[#ADBC9F]/40", text: isDark ? "text-white" : "text-[#12372A]", border: isDark ? "border-white/20" : "border-[#ADBC9F]" };
    case "0-1":
    ...
    default:
      return { label: "—", bg: isDark ? "bg-white/5" : "bg-[#FBFADA]/70", text: isDark ? "text-white/40" : "text-[#436850]", border: isDark ? "border-white/10" : "border-[#ADBC9F]" };
  }
}
...
function AccuracyBar({ label, accuracy, isDark }: { label: string; accuracy: number | null; isDark: boolean }) {
  ...
  const muted = isDark ? "text-white/40" : "text-[#436850]";
  const textColor = isDark ? "text-white/70" : "text-[#12372A]/85";
  return (
    <div className="flex items-center gap-2 min-w-0">
      <span className={`text-[10px] font-medium w-10 shrink-0 truncate ${muted}`}>{label}</span>
      <div className={`flex-1 h-1.5 rounded-full overflow-hidden ${isDark ? "bg-white/10" : "bg-[#ADBC9F]"}`}>
