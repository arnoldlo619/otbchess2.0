...
export function AppNavBar({ defaultActive = "Tournaments", onSignInClick, className }: AppNavBarProps) {
  ...
  const activeStatus = activeTournament
    ? getTournamentStatusDisplay(activeTournament.status)
    : null;
  const showActiveDot = !!activeStatus && (activeStatus.isLive || activeStatus.isPending);
    ...
    { name: "Tournaments",
      ...
      badge: showActiveDot ? (
        <span
          ...
          style={{ animation: activeStatus?.isLive ? "pulse 2s cubic-bezier(0.4,0,0.6,1) infinite" : "none" }}
        />
      ) : undefined,
