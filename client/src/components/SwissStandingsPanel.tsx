...
function seedColor(seed: number, isDark: boolean): string {
  ...
  return isDark ? "bg-white/06 text-white/40 border-white/08" : "bg-[#FBFADA]/70 text-[#436850] border-[#ADBC9F]";
}
...
export function SwissStandingsPanel({
  ...
  return (
    ...
    >
      ...
      >
        <div className="flex items-center gap-3">
          <div className={`w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0 ${
            ...
            <List className={`w-4 h-4 ${isDark ? "text-[#4CAF50]" : "text-[#436850]"}`} />
          </div>
        ...
        <div className={`flex-shrink-0 ${isDark ? "text-white/30" : "text-[#436850]"}`}>
      ...
      {/* Body */}
      {!collapsed && (
        <div className={`border-t ${isDark ? "border-white/06" : "border-[#ADBC9F]/70"}`}>
          {/* Column headers */}
          <div className={`grid grid-cols-[2rem_1fr_3.5rem_3.5rem] gap-x-2 px-5 py-2 ${
            isDark ? "bg-white/02" : "bg-[#FBFADA]/70/80"
          }`}>
            <span className={`text-[10px] font-bold uppercase tracking-wider ${isDark ? "text-white/25" : "text-[#436850]"}`}>#</span>
            <span className={`text-[10px] font-bold uppercase tracking-wider ${isDark ? "text-white/25" : "text-[#436850]"}`}>Player</span>
            <span className={`text-[10px] font-bold uppercase tracking-wider text-right ${isDark ? "text-white/25" : "text-[#436850]"}`}>Pts</span>
            <span className={`text-[10px] font-bold uppercase tracking-wider text-right ${isDark ? "text-white/25" : "text-[#436850]"}`}>Buch.</span>
          </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="min-w-0 flex items-center gap-2">
                    <span className={`text-sm font-semibold truncate ${isDark ? "text-white/90" : "text-[#12372A]"}`}>
                      {row.player.name}
                    </span>
                    {row.player.elo && (
                  ...
                  {/* Points */}
                  <span className={`text-sm font-black tabular-nums text-right ${isDark ? "text-white/85" : "text-[#12372A]"}`}>
                    {row.points % 1 === 0 ? row.points.toFixed(0) : row.points.toFixed(1)}
                  </span>

                  {/* Buchholz */}
                  <span className={`text-xs font-semibold tabular-nums text-right ${isDark ? "text-white/35" : "text-[#436850]"}`}>
          ...
          {eliminated.length > 0 && (
            <>
              <div className={`flex items-center gap-3 px-5 py-2 ${
                isDark ? "bg-white/02 border-t border-white/06" : "bg-[#FBFADA]/70/80 border-t border-[#ADBC9F]/70"
              }`}>
                <span className={`text-[10px] font-bold uppercase tracking-wider ${isDark ? "text-white/20" : "text-[#436850]/70"}`}>
                  Did not advance
                </span>
                <div className={`flex-1 h-px ${isDark ? "bg-white/06" : "bg-[#ADBC9F]"}`} />
              </div>
              <div className={`divide-y ${isDark ? "divide-white/04" : "divide-gray-50"}`}>
                  ...
                  return (
                    ...
                    >
                      <span className={`inline-flex items-center justify-center w-6 h-6 rounded-lg text-[10px] font-bold border ${
                        isDark ? "bg-white/04 text-white/30 border-white/06" : "bg-[#FBFADA]/70 text-[#436850]/70 border-[#ADBC9F]"
                      }`}>
                        {rank}
                      </span>
                      <div className="min-w-0 flex items-center gap-2">
                        <span className={`text-sm font-medium truncate ${isDark ? "text-white/60" : "text-[#436850]"}`}>
                          {row.player.name}
                        </span>
                      </div>
                      <span className={`text-sm font-semibold tabular-nums text-right ${isDark ? "text-white/50" : "text-[#436850]"}`}>
                        {row.points % 1 === 0 ? row.points.toFixed(0) : row.points.toFixed(1)}
                      </span>
                      <span className={`text-xs font-medium tabular-nums text-right ${isDark ? "text-white/25" : "text-[#436850]/70"}`}>
                        {row.buchholz.toFixed(1)}
                      </span>
                    </div>
                  );
              ...
              {eliminated.length > 3 && (
                <div className={`px-5 py-3 border-t ${isDark ? "border-white/06" : "border-[#ADBC9F]/70"}`}>
                  <button
                    onClick={() => setShowAll((s) => !s)}
                    className={`text-xs font-semibold transition-colors ${
                      isDark ? "text-white/30 hover:text-white/55" : "text-[#436850] hover:text-[#436850]"
                    }`}
                  >
                    {showAll
                      ? "Show less"
