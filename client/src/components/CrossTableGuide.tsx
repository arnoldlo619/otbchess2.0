...
export function CrossTableGuide({ isDark }: Props) {
  ...
  function cellColor(val: string | null) {
    ...
    if (val === "0") return isDark ? "text-white/30" : "text-[#6B6B50]/70";
    if (val === "—") return isDark ? "text-white/15" : "text-[#6B6B50]/50";
    return isDark ? "text-white/08" : "text-[#6B6B50]/50";
  }
  ...
  return (
    <div className="space-y-8">
        ...
        <div className="p-5">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {[
              ...
              { symbol: "0", label: "Loss", desc: "You lost to this opponent", color: isDark ? "text-white/40" : "text-[#6B6B50]", bg: isDark ? "bg-white/06" : "bg-[#FFF3D5]/70" },
              { symbol: "—", label: "Diagonal / Not Played", desc: "Can't play yourself, or no game this round", color: isDark ? "text-white/20" : "text-[#6B6B50]/70", bg: isDark ? "bg-white/04" : "bg-[#FFF3D5]/70" },
            ].map((item) => (
              <div key={item.symbol} className={`guide-highlight rounded-xl p-4 text-center ${item.bg}`}>
                ...
                <p className={`text-sm font-bold ${isDark ? "text-white" : "text-[#1A1A1A]"}`}>{item.label}</p>
          ...
          <table className="w-full text-sm border-collapse">
            <thead>
              <tr className={thead}>
                ...
                <th className={`${th} text-center ${isDark ? "text-[#4CAF50]" : "text-[#4D6940]"}`}>Pts</th>
              </tr>
            </thead>
            <tbody>
              {players.map((row, i) => (
                <tr key={i} className={tdBorder}>
                  <td className={`px-3 py-3 text-xs font-bold text-center ${isDark ? "text-white/30" : "text-[#6B6B50]"}`}>{row.rank}</td>
                  ...
                  <td className={`px-3 py-3 text-center font-bold tabular-nums ${isDark ? "text-[#4CAF50]" : "text-[#4D6940]"}`}>
                    {row.pts}
                  </td>
                </tr>
              ))}
      ...
      <div className={card}>
        ...
        <div className="p-5 space-y-3">
          {[
            ...
            { row: "cdchi94 (#2)", col: "Column 1 (Ken)", value: "0", meaning: "cdchi94 lost to Ken", color: isDark ? "text-white/40" : "text-[#6B6B50]" },
            { row: "Ken (#1)", col: "Column 1 (self)", value: "—", meaning: "Diagonal — a player can't play themselves", color: isDark ? "text-white/20" : "text-[#6B6B50]/70" },
            { row: "Kyle Hugo (#4)", col: "Column 1 (Ken)", value: "(empty)", meaning: "These two never played each other in this tournament", color: isDark ? "text-white/20" : "text-[#6B6B50]/70" },
          ].map((item, i) => (
            <div key={i} className={`guide-highlight flex items-start gap-3 rounded-xl px-4 py-3 ${isDark ? "bg-white/04" : "bg-[#FFF3D5]/70"}`}>
      ...
      {/* ── Symmetry rule ── */}
      <div className={`guide-highlight rounded-2xl border p-5 ${isDark ? "border-[#4CAF50]/20 bg-[#4D6940]/10" : "border-[#4D6940]/20 bg-[#4D6940]/04"}`}>
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className={`text-sm mt-1 ${isDark ? "text-white/60" : "text-[#6B6B50]"}`}>
            ...
            <p className={`text-xs mt-2 font-mono ${isDark ? "text-white/30" : "text-[#6B6B50]"}`}>
              ...
              <tbody>
                {[
                  ...
                  <tr key={i} className={tdBorder}>
                    ...
                    <td className={`px-3 py-3 text-center font-bold tabular-nums ${isDark ? "text-[#4CAF50]" : "text-[#4D6940]"}`}>{row.pts}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {/* ── Swiss pairing note ── */}
      <div className={`guide-card rounded-2xl border p-5 ${isDark ? "border-white/08" : "border-[#E8D9B0]/70"}`}>
        <div className="flex items-start gap-3">
