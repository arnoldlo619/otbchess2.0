...
export function SpectatorTimerBanner({ snap }: Props) {
  ...
  const bgClass = isExpired
    ...
    : isDark ? "bg-[oklch(0.22_0.06_145)] border-white/10" : "bg-[#FBFADA] border-[#EEEED2]";
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <p className={`text-xs font-semibold uppercase tracking-widest mb-1 ${isDark ? "text-white/40" : "text-[#436850]"}`}>
          {statusLabel}
        </p>
        <p
      ...
      <div className="hidden sm:flex flex-col items-end flex-shrink-0">
        ...
        <p className={`text-xs ${isDark ? "text-white/30" : "text-[#436850]"}`}>remaining</p>
      </div>
    </div>
  );
}
